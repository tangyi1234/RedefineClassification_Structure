...
{
//    NSLog(@"这里监控的数据:%@",[change objectForKey:NSKeyValueChangeOldKey]);
    NSLog(@"这里监控的数据1:%@",[change objectForKey:NSKeyValueChangeNewKey]);
...
//    }
    if (self.block != nil)
    {
        id oldValue = [change objectForKey:NSKeyValueChangeOldKey];//之前的状态值
        if (oldValue == [NSNull null])
        {
            oldValue = nil;
        }
        
        id newValue = [change objectForKey:NSKeyValueChangeNewKey];//当前的状态值
